...
function Ae(e, t) {
  return t.key !== Se ? G(Pe(e, t.key), t) : t.op === "missing" ? ve(e).every((n) => !G(n, { ...t, op: "exists" })) : ve(e).some((n) => G(n, t));
}
function B(e) {
...
function D(e) {
  ...
  const s = (o, l, a) => {
    ...
    for (const u in f)
      u.startsWith("$") || a === 0 && Ke.has(u.toLowerCase()) || s(f[u], l ? l + "." + u : u, a + 1);
  };
  try {
...
function Ue(e, t, n, s) {
  const o = D(e);
  for (const l in o) {
    const a = l.toLowerCase(), d = Ye(l).toLowerCase();
...
function $e(e, t, n) {
  ...
  return d && (s ? d : d.toLowerCase()).includes(t) ? "тип: " + d : Fe(e, t, s);
}
async function qe(e, t, n) {
  ...
  const f = Ee(e, t.includeHidden);
  for (const u of f) {
    const R = [];
    u.drawing.layers.forEach((S) => {
      R.push(S);
    });
    for (const S of R) {
      d++, d % _e === 0 && (n?.(d, a.length, u.title), await we());
      const j = $e(S, l, t);
      j && a.push({
        index: a.length,
        layer: S,
  ...
  for (let u = 0; u < a.length; u++)
    a[u].props = D(a[u].layer), u % 200 === 0 && (n?.(d, a.length, "чтение свойств"), await we());
  return n?.(d, a.length, ""), { hits: a, scanned: d, models: f.length, elapsed: Date.now() - s };
}
function we() {
  return new Promise((e) => setTimeout(e, 0));
}
const ee = "Прочее", I = "Элемент";
function te(e) {
  const t = e.lastIndexOf("|");
...
function We(e) {
  ...
  const n = [...t.entries()].filter(([s]) => s !== I).sort((s, o) => s[0] === ee ? 1 : o[0] === ee ? -1 : s[0].localeCompare(o[0], "ru"));
  return [
    { group: I, rows: t.get(I) },
...
function ne(e, t, n) {
  ...
  if (s.layer.clearSelected(), o.size && s.layer.selectObjects((f) => {
    const u = f;
    return Xe(u?.layer, o) ? (u.qbounds && u.qbounds(a) && (d === 0 ? Math3d.box3.dup(l, a) : Math3d.box3.addBox(l, a), d++), !0) : !1;
  }, !0), d > 0)
    try {
      s.camera.zoom(l, s);
    } catch {
...
function nt(e, t, n, s) {
  ...
  const a = o.querySelector("#app"), d = o.querySelector("#query"), f = o.querySelector("#param"), u = o.querySelector("#param-list"), R = o.querySelector("#param-clear"), S = o.querySelector("#hide"), j = o.querySelector("#show-all"), K = o.querySelector("#case"), Y = o.querySelector("#hidden"), ie = o.querySelector("#find"), ae = o.querySelector("#all"), ce = o.querySelector("#prev"), le = o.querySelector("#next"), Oe = o.querySelector("#reset"), de = o.querySelector("#status"), T = o.querySelector("#list"), E = o.querySelector("#conditions"), Me = o.querySelector("#conditions-count"), Ne = o.querySelector("#add-condition"), F = o.querySelector("#fold-conditions"), ue = o.querySelector("#fold-props"), V = o.querySelector("#props"), pe = o.querySelector("#prop-filter");
  K.checked = l.caseSensitive, Y.checked = l.includeHidden, J(e, a);
  let k = [], g = [], w = [], Ie = 1, q = -1, x = !1, M;
  function h(r, i = !1) {
  ...
  function H() {
    return {
      query: d.value,
      property: f.value,
      caseSensitive: K.checked,
      includeHidden: Y.checked
    };
  }
  function L() {
    const r = g.length > 0;
    ae.disabled = !r || x, ce.disabled = !r || x, le.disabled = !r || x, S.disabled = !r || x, ie.disabled = x, j.disabled = x, a.classList.toggle("scoped", f.value.trim() !== "");
  }
  function je() {
    ...
    u.innerHTML = [...r].sort((i, c) => i.localeCompare(c, "ru")).slice(0, 400).map((i) => '<option value="' + b(i) + '"></option>').join("");
  }
  function N() {
    const r = be(k);
    if (Me.textContent = w.length ? "· " + w.length : "", !w.length) {
      E.innerHTML = '<div class="empty">Условий нет. Кнопка ＋ добавит условие по свойству.</div>';
      return;
    }
    const i = /* @__PURE__ */ new Map();
    for (const c of r) {
      const { group: p, name: m } = te(c), v = ["Имя", "Модель", "Путь"].includes(c) ? I : p, y = v === I ? c : m, C = i.get(v);
      C ? C.push({ key: c, name: y }) : i.set(v, [{ key: c, name: y }]);
    }
    E.innerHTML = w.map((c) => {
      const p = ['<option value="">любое свойство</option>'].concat([...i.entries()].map(
        ([O, Ve]) => '<optgroup label="' + b(O) + '">' + Ve.map(
      ...
      return '<div class="condition' + (C ? " waiting" : "") + '" data-id="' + c.id + '"><select class="cond-key">' + p + '</select><select class="cond-op">' + m + '</select><input class="cond-value" type="search" placeholder="' + (y ? "число" : "значение") + '" value="' + b(c.value) + '"' + (v ? "" : " disabled") + '><button class="cond-remove" title="Удалить условие">×</button>' + (he ? '<div class="cond-hint">' + he + ", условие пока не применяется</div>" : "") + "</div>";
    }).join("");
  }
  function Te() {
    if (!g.length) {
      T.innerHTML = '<div class="empty">' + (k.length ? "Условия отбора не пропустили ни одного элемента" : "Пока ничего не найдено") + "</div>";
      return;
    }
    T.innerHTML = g.map(
  ...
  function P() {
    const r = M ?? g.find((m) => m.index === q);
    if (!r) {
      V.innerHTML = '<div class="empty">Выберите элемент в списке или в модели</div>';
      return;
    }
    const i = pe.value.trim().toLowerCase(), c = We(r).map((m) => ({
      group: m.group,
    ...
    const p = M ? '<div class="props-note">Элемент выбран в модели</div>' : "";
    V.innerHTML = p + c.map(
  ...
  function fe() {
    return g.findIndex((r) => r.index === q);
  }
  async function $() {
    const r = H();
    ...
    Je(t) || h("Нет активного вида чертежа. Поиск выполнится, но подсветка работать не будет."), x = !0, k = [], g = [], q = -1, L(), h("Поиск…");
    try {
      const c = await qe(i, r, (p, m, v) => {
        h("Просмотрено " + p + ", найдено " + m + (v ? ". " + v : ""));
      });
      if (k = c.hits, N(), je(), A(!0), k.length) {
        const p = w.filter(B).length ? " После условий отбора: " + g.length + "." : "";
        h("Найдено: " + k.length + "." + p + " Просмотрено слоёв: " + c.scanned + " в моделях: " + c.models + " за " + Math.round(c.elapsed / 100) / 10 + " с.");
      } else {
        const p = r.property.trim() ? " Параметр: " + r.property.trim() + "." : "";
        h("Ничего не найдено." + p + " Просмотрено слоёв: " + c.scanned + " в моделях: " + c.models + ".");
      }
    } catch (c) {
  ...
  ie.addEventListener("click", () => {
    ...
    f.value = "", z(H()), L(), f.focus();
  }), K.addEventListener("change", () => z(H())), Y.addEventListener("change", () => z(H())), S.addEventListener("click", () => {
    (async () => {
      if (g.length) {
        x = !0, L(), h("Скрываю элементы…");
        try {
          const r = await Qe(g.map((i) => i.layer));
    ...
    if (g.length) {
      ...
      q = -1, _(-1), h("Подсвечено элементов: " + g.length + ".");
    }
  }), ce.addEventListener("click", () => {
    if (!g.length) return;
    const r = fe();
    U(r <= 0 ? g.length - 1 : r - 1);
  }), le.addEventListener("click", () => {
    if (!g.length) return;
    const r = fe();
    U(r >= g.length - 1 ? 0 : r + 1);
  }), Oe.addEventListener("click", () => {
    oe(t), q = -1, _(-1), h("Выделение снято.");
  }), T.addEventListener("click", (r) => {
    const i = r.target.closest(".row");
    if (!i) return;
    const c = Number(i.dataset.index);
    U(g.findIndex((p) => p.index === c));
  }), Ne.addEventListener("click", () => {
    w.push({ id: Ie++, key: Se, op: "contains", value: "" }), E.hidden = !1, F.setAttribute("aria-expanded", "true"), N(), A();
  }), F.addEventListener("click", () => {
    const r = E.hidden;
    E.hidden = !r, F.setAttribute("aria-expanded", String(r)), r && N();
  }), ue.addEventListener("click", () => {
    const r = V.hidden;
    V.hidden = !r, ue.setAttribute("aria-expanded", String(r));
  }), pe.addEventListener("input", () => P()), E.addEventListener("click", (r) => {
    const i = r.target.closest(".cond-remove");
    if (!i) return;
    const c = Number(i.closest(".condition").dataset.id);
    w = w.filter((p) => p.id !== c), N(), A();
  }), E.addEventListener("change", (r) => {
    const i = r.target, c = i.closest(".condition");
    if (!c) return;
    const p = w.find((m) => m.id === Number(c.dataset.id));
    p && (i.classList.contains("cond-key") && (p.key = i.value), i.classList.contains("cond-op") && (p.op = i.value, N()), i.classList.contains("cond-value") && (p.value = i.value), A());
  }), E.addEventListener("input", (r) => {
    const i = r.target;
    if (!i.classList.contains("cond-value")) return;
    const c = i.closest(".condition");
    if (!c) return;
    const p = w.find((C) => C.id === Number(c.dataset.id));
    if (!p) return;
    p.value = i.value;
    const m = p.op === "gt" || p.op === "lt", v = !B(p);
    c.classList.toggle("waiting", v);
    let y = c.querySelector(".cond-hint");
    v ? (y || (y = document.createElement("div"), y.className = "cond-hint", c.appendChild(y)), y.textContent = (m && p.value.trim() !== "" ? "нужно число" : "введите значение") + ", условие пока не применяется") : y && y.remove(), A();
  });
  const ge = (r) => {
    if (!e.isConnected) {
      re.delete(ge);
    ...
    const i = r[0], c = g.find((p) => p.layer === i);
    if (c) {
      _(c.index);
      return;
    }
...
const st = {
  ...
  mount(e) {
    ...
    n.style.height = "100%", t.replaceChildren(n), nt(n, X(e), ot, rt);
  },
